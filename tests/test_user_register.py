...
class TestUserRegister(BaseCase):
    ...
    def test_create_user_successfully(self):
        ...
        with allure.step(f"actual response code = {response.status_code} actual response body = {response.content}"):
    ...
    def test_create_user_with_existing_email(self):
         ...
         with allure.step(f"create user with existing={email}"):
             response = MyRequests.post("/user/", data=data)

         with allure.step(
                     f"actual response code = {response.status_code} actual response body = {response.content}"):
    ...
    def test_create_user_with_invalid_email(self): # Создание пользователя с некорректным email - без символа @
        ...
        data = self.prepare_registration_data(email)
        with allure.step(f"create user with invalid email={email}"):
            response = MyRequests.post("/user/", data=data)
    ...
    def test_create_user_with_short_username(self):  # Создание пользователя с очень коротким именем в один символ
        ...
        with allure.step(f"create user with long username={new_value}"):
            response = MyRequests.post("/user/", data=data)
        with allure.step(f"actual response code = {response.status_code} actual response body = {response.content}"):
            Assertions.assert_code_status(response, 400)
    ...
    def test_create_user_with_long_username(self):  # Создание пользователя с очень длинным именем - длиннее 250 символов
        ...
        data = self.prepare_registration_data()
        data['username'] = new_value
        with allure.step(f"create user with long username"):
            response = MyRequests.post("/user/", data=data)
    ...
    def test_with_excluded_param(self, condition):
        with allure.step(f"Test with excluded {condition}"):
            data = self.prepare_registration_data()
